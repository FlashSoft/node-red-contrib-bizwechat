...
const QI_WX_HOST = "https://qyapi.weixin.qq.com";
...
class WechatClass {
  ...
  dealUrl(path) {
    const { proxyWx } = this.config;
    if (proxyWx && proxyWx.startsWith('http')) {
      if (proxyWx.endsWith('/')) {
        return `${proxyWx}${path}`;
      }else {
        return `${proxyWx}/${path}`;
  ...
  getToken () {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const { data } = await axios.get(this.dealUrl('cgi-bin/gettoken'), {
          params: { corpid, corpsecret }
        }).catch(err => {
          console.log(err)
          throw new Error(`[微信Token]:${err.message}`)
        })
        if (data.errcode != 0) {
  ...
  getMedia (media_id) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const result = await axios.get(this.dealUrl('cgi-bin/media/get'), {
          params: { access_token, media_id },
          responseType: 'arraybuffer'
        }).catch(err => {
          throw new Error(`[微信媒体]${err.message}`)
        })
  ...
  uploadMedia (data) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // console.log(header)
        const result = await axios.post(this.dealUrl('cgi-bin/media/upload'), form, {
  ...
  pushMessage (sendData) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const result = await axios.post(this.dealUrl('cgi-bin/message/send'), sendData, {
          params: { access_token }
        }).catch(err => {
